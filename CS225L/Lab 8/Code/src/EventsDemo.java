 ...
 *         Based on original code by Sean Holden (holdens@my.erau.edu), October
 *         2014 *
 * 
 * 
 *         Edited By: Jeremiah Webb Assignment: Lab 8
 * 
 * 
 */
public class EventsDemo extends Application {
	...
	public Boolean checkAnswer(String answer) {
		...
		if (circle.isVisible() && line.isVisible() && triLineLeft.isVisible() && triLineRight.isVisible()
				&& triLineBottom.isVisible()) {
		...
		if (circle.isVisible()) {
			if (answer.equalsIgnoreCase("circle")) {
				answerLabel.setText("This is a Circle!");
				return true;
			}
		}
		if (line.isVisible()) {
			if (answer.equalsIgnoreCase("line")) {
				answerLabel.setText("This is a Line!");
				return true;
			}
		}
		if (triLineLeft.isVisible() && triLineRight.isVisible() && triLineBottom.isVisible()) {
			if (answer.equalsIgnoreCase("triangle")) {
				answerLabel.setText("This is a Triangle!");
				return true;
			}

		}
		answerLabel.setText("That is incorrect!");
		return false;
	}
}
